...
server.post('/complaint',(req,res)=>{
    ...
    const sql = `INSERT INTO complaints (username, id, type_of_complaint, prblm, description, material, year, section, department) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
const values = [user, id, complaint, prblm, desc, material, year, section, department];
...
server.get('/graphData', (req, res) => {
  ...
  server.get("/solved_complaints",(req,res)=>{
    ...
    let query=con.query(sql,(err,rows)=>{
        if(err) {alert(err);}
    else{res.render("solved_complaints",{
...
server.put('/complaints/:id/status', (req, res) => {
    const complaintId = req.params.id;
    const newStatus = req.body.status;
    connection.query('UPDATE all_complaints SET status = ? WHERE id = ?', [newStatus, complaintId], (err, result) => {
      if (err) {
      ...
      if (newStatus === 'solved') {
        connection.query('INSERT INTO solved_complaints SELECT * FROM all_complaints WHERE id = ?', [complaintId], (err, insertResult) => {
          if (err) {
            console.error('Error moving complaint to solved_complaints:', err);
            res.status(500).json({ error: 'Error moving complaint to solved_complaints' });
            return;
          }
  
          // Delete the complaint from the 'all_complaints' table
          connection.query('DELETE FROM all_complaints WHERE id = ?', [complaintId], (err, deleteResult) => {
            if (err) {
              console.error('Error deleting complaint from all_complaints:', err);
              res.status(500).json({ error: 'Error deleting complaint from all_complaints' });
              return;
            }
            res.status(200).json({ message: 'Complaint has been updated and moved to solved_complaints successfully' });
          });
        });
      }
      else{
